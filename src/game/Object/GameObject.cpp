...
bool GameObject::IsVisibleForInState(Player const* u, WorldObject const* viewPoint, bool inVisibleList) const
{
    ...
    float visibleDistance = GetMap()->GetVisibilityDistance() + (inVisibleList ? World::GetVisibleObjectGreyDistance() : 0.0f);//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
...
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â        /* TODO: implement trap stealth, take look at spell 2836
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â        if(GetGOInfo()->type == GAMEOBJECT_TYPE_TRAP && GetGOInfo()->trap.stealthed && u->IsHostileTo(GetOwner()))
        // implementing armed trap stealth, basing on the spell 2836 structure//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_TRAP && GetGOInfo()->trap.stealthed && GetGoState() == GO_STATE_READY)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
        {
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â            if(check stuff here)
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â                return false;
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â        }*/
            Unit *owner = GetOwner();//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
            if (!owner || u->IsHostileTo(owner))//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
            {//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                visibleDistance = 10.5f;//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                //2^3=8 and 300 - from spell 2836, EFFECT_INDEX_1 - SPELL_AURA_MOD_INVISIBILITY_DETECTION; TODO check 200 and improve//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                if (u->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_INVISIBILITY_DETECTION, 8) < 200)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                {//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    if (u->getClass() != CLASS_ROGUE)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                        return false;       // a wild or enemy trap cannot be seen by non-rogues without proper invis detection//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    visibleDistance = 0.0f; // minimal detection distance, will be normalized below//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                }//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â

//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â        // Smuggled Mana Cell required 10 invisibility type detection/state
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â        if (GetEntry() == 187039 && ((u->m_detectInvisibilityMask | u->m_invisibilityMask) & (1 << 10)) == 0)
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â            { return false; }
                if (owner)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                {//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    // apply to the "owner" and "u" the rules for usual stealth detection; the fragment is taken from Unit::IsVisibleForOrDetect//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    // Visible distance based on stealth value (stealth rank 4 300MOD, 10.5 - 3 = 7.5)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    visibleDistance -= (owner->getLevel() / 20.0f);  // for rogue stealth (4 spells): modifier = 5*level//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â

                    // Visible distance is modified by//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    //-Level Diff (every level diff = 1.0f in visible distance)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    visibleDistance += int32(u->GetLevelForTarget(owner)) - int32(owner->GetLevelForTarget(u));//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                }//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â

                //-Stealth Detection(negative like paranoia)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                visibleDistance += (int32(u->GetTotalAuraModifier(SPELL_AURA_MOD_STEALTH_DETECT))) / 5.0f;//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â

                // normalize visible distance//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                if (visibleDistance > MAX_PLAYER_STEALTH_DETECT_RANGE)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    visibleDistance = MAX_PLAYER_STEALTH_DETECT_RANGE;//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                else if (visibleDistance < GetGOInfo()->trap.radius + INTERACTION_DISTANCE)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
                    visibleDistance = GetGOInfo()->trap.radius + INTERACTION_DISTANCE;//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
            }//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
        }//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â

        // [-ZERO] Smuggled Mana Cell required 10 invisibility type detection/state//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
        //if (GetEntry() == 187039 && ((u->m_detectInvisibilityMask | u->m_invisibilityMask) & (1 << 10)) == 0)//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
        //    { return false; }//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
    }

    // check distance
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â    return IsWithinDistInMap(viewPoint, GetMap()->GetVisibilityDistance() +
//…æ≥˝¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â                             (inVisibleList ? World::GetVisibleObjectGreyDistance() : 0.0f), false);
    return IsWithinDistInMap(viewPoint, visibleDistance, false);//ÃÌº”¥˙¬Î–ﬁ∏¥’Ï≤‚œ›⁄Â
}
