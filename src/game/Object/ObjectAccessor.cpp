...
{
//É¾³ý´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî    HashMapHolder<Player>::ReadGuard g(HashMapHolder<Player>::GetLock());
//É¾³ý´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî    HashMapHolder<Player>::MapType& m = sObjectAccessor.GetPlayers();
//É¾³ý´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî    for (HashMapHolder<Player>::MapType::iterator itr = m.begin(); itr != m.end(); ++itr)
//É¾³ý´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî        { itr->second->SaveToDB(); }
   SessionMap const& smap = sWorld.GetAllSessions();//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
   SessionMap::const_iterator iter;//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
   for (iter = smap.begin(); iter != smap.end(); ++iter){//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
       if (Player* player = iter->second->GetPlayer()){//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
           if (player->IsInWorld()){//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
               player->SaveToDB();//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
           }//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
       }//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
   }//Ìí¼Ó´úÂëÐÞ¸´µ¼ÖÂ±ÀÀ£µÄgmÃüÁî
}
